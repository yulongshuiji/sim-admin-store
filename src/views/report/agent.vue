<template>
  <div class="app-container">
    ...
    <!-- 下线用户弹窗 -->
    <el-dialog title="下线会员" :visible.sync="openUser" width="90%" :close-on-click-modal="false" append-to-body>
      <UserDialog  :key="timerUser" :parentId="parentId" :dateTime="dateTime" :currencyId="queryParams.currency_id" :counterId="queryParams.counter_id" />
    </el-dialog>

    <!-- 注单弹窗 -->
    <el-dialog title="注单" :visible.sync="openBet" width="90%" :close-on-click-modal="false" append-to-body>
      <BetDialog :key="timerBet" :parentId="parentId" :dateTime="dateTime" :currencyId="queryParams.currency_id" :counterId="queryParams.counter_id" />
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    // 下线会员
    handleUser(row){
      this.parentId = row.user_id
      this.timerUser = new Date().getTime()
      this.openUser = true
    },
    // 注单
    handleBet(row){
