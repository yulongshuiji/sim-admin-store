    ...
    <div class="content">
      ...
      <el-form :model="queryParams" ref="queryForm" :inline="true">
        <el-form-item label="账号" prop="admin_name">
          <el-input v-model="queryParams.admin_name" placeholder="请输入账号" clearable size="small" style="width: 150px"
            @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item label="姓名" prop="admin_nickname">
          <el-input v-model="queryParams.admin_nickname" placeholder="请输入姓名" clearable size="small" style="width: 150px"
            @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item label="手机号码" prop="phone">
          <el-input v-model="queryParams.phone" placeholder="请输入手机号码" clearable size="small" style="width: 150px"
            @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item label="角色" prop="role_id">
          <el-select v-model="queryParams.role_id" placeholder="请选择角色" clearable size="small" style="width: 150px">
            <el-option v-for="item in queryRoles" :key="item.role_id" :label="item.role_name" :value="item.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button type="success" icon="el-icon-plus" size="mini" @click="handleAdd"
            v-hasPermi="['permission:admin:add']">新增</el-button>
        </el-form-item>
      </el-form>
      ...
      <el-table v-loading="loading" :data="listData" border max-height="650">
        ...
        <el-table-column label="角色" align="center">
          <template slot-scope="scope">
            <el-tag v-for="item in scope.row.role_name" :key="item" type="info" style="margin-right: 8px;">{{ item
              }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="280">
          <template slot-scope="scope">
            <el-button v-if="scope.row.root != 1" size="mini" type="text" icon="el-icon-edit"
              @click="handleUpdate(scope.row)" v-hasPermi="['permission:admin:update']">编辑</el-button>
            <el-button v-if="scope.row.root != 1" size="mini" type="text" icon="el-icon-edit"
              @click="toLinkFingerprint(scope.row)" v-hasPermi="['permission:admin:fingerprint']">指纹录取</el-button>
            <el-button v-if="scope.row.root != 1" size="mini" type="text" icon="el-icon-delete"
              @click="handleDelete(scope.row)" v-hasPermi="['permission:admin:delete']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <!-- 添加修改用户对话框 -->
      <el-dialog :title="title" :visible.sync="open" v-if="open" width="50%" :close-on-click-modal="false"
        append-to-body>
            ...
            <el-col :span="12">
              <el-form-item label="角色" prop="role_id">
                <el-select v-model="form.role_id" multiple :multiple-limit="1" placeholder="请选择角色">
                  <el-option v-for="item in formRoles" :key="item.role_id" :label="item.role_name" :value="item.role_id"
                    :disabled="item.role_id == 1"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="状态" prop="status">
                <el-radio-group v-model="form.status">
                  <el-radio v-for="item in statusOptions" :key="item.value"
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="备注" prop="remark">
                <el-input v-model="form.remark" maxlength="30" rows="2" show-word-limit resize="none" type="textarea"
      ...
      <!-- 添加修改用户对话框 -->
      <el-dialog :show-close="false" title="指纹录取" :visible.sync="openFingerprint" v-if="openFingerprint" width="400px"
